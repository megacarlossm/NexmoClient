        ...
        return new Promise((resolve, reject) => {
            ...
            this.rtcHelper.getUserAudio(params.audioConstraints).then((localStream) => {
                ...
                const do_gatherDone = async () => {
                    ...
                    sdpOfferNewObj.media[0] ? sdpOfferNewObj.media[0].candidates = [candidate] : sdpOfferNewObj.media[0] = { candidates: [candidate] };
                    sdpOfferNewObj.media[0].candidates = [candidate];
                    const sdpOfferNew = sdptransform.write(sdpOfferNewObj);
                    offer = { sdp: sdpOfferNew, type: "offer" };
                    try {
                ...
                if (this.application.session.config.iceGatherOnlyOneCandidate) {
                    pc.onicecandidate = (event) => {
                        if (event.candidate) {
                            // @ts-ignore
                            if (isIp.v4(event.candidate.address)) {
                    ...
                    pc.onicegatheringstatechange = () => {
                        switch (pc.iceGatheringState) {
                            ...
                            case 'complete':
                                ...
                                break;
                            case 'gathering':
                                //Wait 2 seconds for complete and if still gathering send offer
